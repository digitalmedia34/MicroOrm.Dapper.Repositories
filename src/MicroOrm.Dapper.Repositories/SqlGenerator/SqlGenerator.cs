...
namespace MicroOrm.Dapper.Repositories.SqlGenerator
{
    ...
    public class SqlGenerator<TEntity> : ISqlGenerator<TEntity> where TEntity : class
    {
        ...
        /// <inheritdoc />
        public virtual SqlQuery GetSelectFirst(Expression<Func<TEntity, bool>> predicate, bool firstOnly = false, params Expression<Func<TEntity, object>>[] includes)
        {
            return GetSelect(predicate, firstOnly, includes);
        }
        ...
        /// <inheritdoc />
        public SqlQuery GetSelectById(object id, bool firstOnly, params Expression<Func<TEntity, object>>[] includes)
        {
        ...
        #region Bulk Upsert
            ...
            private string GetTempTableInsert(IEnumerable<TEntity> entities, string tempTableName)
            {
                var entitiesArray = entities as TEntity[] ?? entities.ToArray();
                if (!entitiesArray.Any())
                    throw new ArgumentException("collection is empty");
                ...
                values.Add("(" + string.Join(", ", properties.Select(p => "source." + p.PropertyName)) + ")");
            ...
            private string GetTempTableUpdate(IEnumerable<TEntity> entities)
            {
                var properties = (IsIdentity ? SqlProperties.Where(p => !p.PropertyName.Equals(IdentitySqlProperty.PropertyName, StringComparison.OrdinalIgnoreCase))
                    : SqlProperties).ToList();
                ...
                if (HasModifiedAt)
                {
                    update = "UPDATE SET " +
                             string.Join(", ", properties.Where(e => e.PropertyName != UpdatedAtProperty?.Name)
                                 .Select(p => "target." + p.ColumnName + " = source." + p.ColumnName));
                }
                else
                {
                    //exclude both modified and updated at
